...
export function ReportLibraryCard({
  ...
  const { membershipContent } = useCommerceRegion();
      ...
      <CardHeader className="space-y-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap justify-end gap-2">
            ...
            {hasActiveMembership ? (
              <Badge variant="subtle">{membershipContent.membershipNarrative.libraryBadge}</Badge>
            ) : null}
          </div>
        </div>
        <div className="space-y-3">
          ...
          <p className="body-sm max-w-3xl">
            Saved to your library with download and email access from the same place.
          </p>
          {hasActiveMembership ? (
            <p className="text-sm leading-6 text-muted">
              Also connected to your active membership library.
            </p>
          ) : null}
        </div>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="grid gap-3 xl:grid-cols-[1.05fr_0.95fr]">
          <div className="surface-block px-5 py-5">
            ...
            <p className="mt-3 reading-column text-sm leading-6 text-foreground">
              {getAccessBadge(report)} access. Your report stays available here after purchase and can be reopened, downloaded, or emailed again anytime.
            </p>
          </div>
          <div className="surface-block-strong px-5 py-5">
            ...
            <div className="mt-4 space-y-3 text-sm leading-6 text-muted">
              ...
              <p>{emailPlan.note}</p>
              <p>Account email: <span className="break-all text-foreground/92">{accountEmail}</span></p>
        ...
        {report.file.failureReason ?? report.delivery.failureReason ? (
          <div className="rounded-[24px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))] px-5 py-4">
            ...
            <p className="mt-2 reading-column text-base text-foreground">
              {report.file.failureReason ?? report.delivery.failureReason}
            </p>
          </div>
        ) : null}
